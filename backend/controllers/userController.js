...
const { uploadBufferToCloudinary, deleteFromCloudinary, getPublicIdFromUrl } = require('../utils/cloudinaryUpload');
  ...
  try {
    upload.single('avatar')(req, res, async (err) => {
      ...
      try {
        ...
        // Clean up the previous avatar so old images don't pile up on Cloudinary
  ...
  try {
    upload.single('resume')(req, res, async (err) => {
      ...
      try {
        ...
        // 'auto' lets Cloudinary handle pdf/doc/docx correctly (they're
        // stored as 'raw' resources, unlike images/videos).
        const result = await uploadBufferToCloudinary(req.file.buffer, {
